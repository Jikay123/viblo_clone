...
function Fan({ data, userLogin }) {
    ...
    useEffect(() => {
                ...
                snap.docs.map(value => {
                    if (value.data().uid === item.uid) {
                        if (valueFollow.findIndex((itemValue) => itemValue.id === value.id) === -1) {
                            if (valueFollowRef.current.findIndex(dataRef => dataRef.id === value.id) === -1) {
        ...
        setTimeout(() => {
            if (data?.length < valueFollowRef.current?.length) {
                const arr = [];
                data.map((dataRef) => {
                    const filter = valueFollowRef.current.filter(item => {
                        return item.data.uid === dataRef.uid
                    })
                    arr.push(...filter)
                    return null;
                })
                console.log(arr, "arr")
                valueFollowRef.current = arr;
            }
            setValueFollow(valueFollowRef.current)
        }, 500)

        return () => {
            valueFollowRef.current = []
            setValueFollow([])
        }
    ...
    const handleFollowUser = () => {
        ...
        if (!userLogin.data.followUser) {
            ...
            const index = userLogin.data.followUser.findIndex(item => item.uid === valueProfile.data.uid)
            if (index === -1) {
        ...
        <> {
            (data && data.length > 0) ? (
                <div className="followed">
                    {valueFollow.length > 0 && valueFollow?.map(item => (
                ...
                <div>{"Người dùng chưa có người theo dõi"}</div>
        }
        </>
    )
}
